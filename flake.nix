  ...
  outputs = { self, nixpkgs, home-manager, ... }:
  {
    homeModules = {
      ...
      in {
        ...
        config = {
          home.activation.installNpmPackages = if config.npm.enable then lib.hm.dag.entryAfter ["writeBoundary"] ''
          ...
          home.activation.installPipxPackages = if config.pipx.enable then lib.hm.dag.entryAfter ["writeBoundary"] ''
          ...
          home.activation.installEgetPackages = if config.eget.enable then lib.hm.dag.entryAfter ["writeBoundary"] ''
